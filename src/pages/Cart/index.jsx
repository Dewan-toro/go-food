  ...
  const columns = [
    {
      ...
      selector: (row) => (
        <Image
          ...
          src={`${config.api_host}/images/products/${row.image_url}`}
          rounded
        />
      ),
    },
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <Button variant="success" size="sm" onClick={() => handleMinus(row)}>
            <FontAwesomeIcon icon={solid("minus")} />
          </Button>
          <span className="mx-4">{row.qty}</span>
          <Button variant="success" size="sm" onClick={() => handlePlus(row)}>
